...
def upload_file():
	...
	filename = get_next_name(
		...
		docname,
		attachment_type,
	) if attachment_type else filename
	if not filename:
	...
	if content is not None and (frappe.session.user == "Guest" or (user and not user.has_desk_access())):
		filetype = guess_type(original_filename)[0]
...
def get_next_name(
	...
	"""Get the next name for the file based on the attachment type."""
	doctype = "Attachment Type"
	doc = frappe.get_doc(doctype, attachment_type)
	return doc.next_name(
