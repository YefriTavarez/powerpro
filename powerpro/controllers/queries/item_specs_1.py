...
def get_item_specs_1(doctype, txt, searchfield="name", start=0, page_len=20, filters=None):
	"""
	Retrieve item specs 1 information based on the search text.

	Args:
		doctype (str): The type of document to search within.
		txt (str): The text to search for within item specs 1.
	...
	Returns:
		list: A list of item specs 1 results matching the search criteria.
	"""
	searchstr = "%%"
	if txt:
		searchstr = f"%{txt}%"
	...
	if "item_name" not in filters:
		frappe.msgprint("Please provide an item name to search for item specs 1.", alert=True)
		return []

	out = frappe.db.sql(
		f"""
			Select
				parent.item_spec1_name
			From
				`tabItem Specs 1` As parent
			Inner Join
				`tabItem Name Link` As child
				On child.parenttype = "Item Specs 1"
					And child.parentfield = "item_names"
					And child.parent = parent.name
			Where
				child.item_name = {filters["item_name"]!r}
				And parent.item_spec1_name Like {searchstr!r}
		""", as_list=True
	)
	return out
