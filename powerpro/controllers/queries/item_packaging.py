...
def get_item_packaging(doctype, txt, searchfield="name", start=0, page_len=20, filters=None):
	"""
	Retrieve item packaging information based on the search text.

	Args:
		doctype (str): The type of document to search within.
		txt (str): The text to search for within item packaging.
	...
	Returns:
		list: A list of item packaging results matching the search criteria.
	"""
	searchstr = "%%"
	if txt:
		searchstr = f"%{txt}%"
	...
	if "item_name" not in filters:
		frappe.msgprint("Please provide an item name to search for item packaging.", alert=True)
		return []

	out = frappe.db.sql(
		f"""
			Select
				parent.item_packaging
			From
				`tabItem Packaging` As parent
			Inner Join
				`tabItem Name Link` As child
				On child.parenttype = "Item Packaging"
					And child.parentfield = "item_names"
					And child.parent = parent.name
			Where
				child.item_name = {filters["item_name"]!r}
				And item_packaging Like {searchstr!r}
		""", as_list=True
	)
	return out
