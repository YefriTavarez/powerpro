...
(() => {
    ...
    async function open(frm) {
        ...
        d.fields_dict.notice.$wrapper.append(`<p>${esc(__(state.planned_settlement))} · ${esc(__("Cash Payroll Date Policy"))}: ${esc(__(state.payroll_date_policy || "Work Date"))}${state.payroll_date ? ` · ${esc(state.payroll_date)}` : ""}</p>`);
        d.fields_dict.records.$wrapper.html(`<div style="overflow:auto"><table class="table table-bordered"><thead><tr>${["Employee / Date","Approved Window / Hours","Attendance","Verified Hours","Presumed Hours","Processing","Settlement","Details"].map(x=>`<th>${esc(__(x))}</th>`).join("")}</tr></thead><tbody>${state.rows.map(r=>`<tr><td>${esc(r.employee_name)}<br>${esc(r.work_date)}</td><td>${esc(r.authorization_start)}<br>${esc(r.authorization_end)}<br>${esc(r.maximum_hours)}</td><td>${esc(__(r.attendance_state || "Pending"))}</td><td>${esc(r.verified_hours)}</td><td>${esc(r.presumed_hours)}</td><td>${esc(__(r.auto_status || "Not enrolled"))}</td><td>${esc(__(r.settlement_status))}</td><td>${esc(r.auto_error)}${r.attendance_exception ? `<br><a href="#" data-overtime-audit="${esc(r.name)}">${esc(__("View Exception"))}</a>` : ""}</td></tr>`).join("")}</tbody></table></div>`);
