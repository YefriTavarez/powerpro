...
powerpro.working_time_controls.add_button = frm => {
    ...
    frm.add_custom_button(__('Controles de jornada'), () => {
        ...
        frappe.prompt([
            ...
            if (frm.is_dirty()) return frappe.msgprint(__('Guarde los cambios antes de consultar los controles.'));
            frappe.call({method:'powerpro.controllers.working_time_controls.preview',args:{source_type:dt,source_name:frm.doc.name,
                ...values,profile:profiles[values.profile],break_rule:breaks[values.break_rule],quarterly_basis:causes[values.quarterly_basis]},freeze:true}).then(({message:r})=>{
                ...
                new frappe.ui.Dialog({title:__('Controles de jornada'),size:'extra-large',fields:[{fieldtype:'HTML',fieldname:'result',options:html}]}).show();
            });
