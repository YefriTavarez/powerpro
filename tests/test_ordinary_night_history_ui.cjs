...
(async()=>{
 ...
 const result={token:'historical-token',note:'<untrusted>',worked_hours_before:8,worked_hours_after:6,
  observation_window:{start:'2026-09-14T18:00:00',end:'2026-09-15T06:00:00'},unapproved_intervals:[{start:'<outside>',end:'2026-09-15T06:00:00'}],
 ...
 prompt.fn({reason:'Historical correction',expand_window:1,observation_start:'2026-09-14 18:00:00',observation_end:'2026-09-15 06:00:00',manual:1,reference:'Signed record',intervals:[{start:'2026-09-14 18:00:00',end:'2026-09-15 01:00:00',idx:1}]});
 ...
 assert.deepEqual(JSON.parse(calls[1].args.observation_window),result.observation_window);
 assert.equal(JSON.parse(calls[0].args.observation_window).end,'2026-09-15 06:00:00');
 assert(dialogs[0].fields[0].options.includes('&lt;outside&gt;'));
