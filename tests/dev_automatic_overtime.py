...
try:
 ...
 def source(key,method,date,start,end):
  ...
  doc=frappe.copy_doc(base);doc.name=prefix+'-'+key;doc.employee=employee.name;doc.employee_name=employee.employee_name;doc.docstatus=1;doc.status='Approved';doc.overtime_work_call=call.name;doc.work_date=date;doc.day_classification='Weekly Rest' if frappe.utils.getdate(date).weekday()==6 else 'Regular Workday';doc.authorization_start=date+' '+start;doc.authorization_end=date+' '+end;doc.maximum_hours=(frappe.utils.get_datetime(doc.authorization_end)-frappe.utils.get_datetime(doc.authorization_start)).total_seconds()/3600;doc.planned_settlement=method;doc.settlement_status='Pending';doc.reconciliation_status='Scheduled';doc.verified_hours=0;doc.reconciled_on=None;doc.reconciliation_source=None;doc.evidence_enrolled=0;doc.db_insert()
