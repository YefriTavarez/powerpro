...
async function fixture(reply,doc={docstatus:1,name:'AUTH',evidence_enrolled:1},dirty=false){
 ...
 const ctx={__:x=>x,powerpro:{checkin_overtime:{}},frappe:{provide(){},call(q){calls.push(q);return Promise.resolve({message:calls.length===1?reply:{status:'Verified'}})},msgprint(x){messages.push(x)},show_alert(){},set_route(...args){routes.push(args)},new_doc(...args){drafts.push(args)}}};
...
(async()=>{
 ...
 assert.equal(x.calls[1].type,'POST');assert.equal(x.calls[1].method,'powerpro.controllers.checkin_overtime.process_now');assert(x.messages.includes('reloaded'));
 x=await fixture({enabled:true,can_process:true,evidence_enrolled:1},undefined,true);x.buttons[0].fn();assert.equal(x.calls.length,1);
 x=await fixture({enabled:true,can_process:true,evidence_enrolled:0},{docstatus:1,name:'AUTH'});x.buttons[0].fn();assert.equal(x.calls[1].method,'powerpro.controllers.checkin_overtime.enroll');
