...
async function fixture(allowed=true){
 ...
 const ctx={__:x=>x,powerpro:{checkin_overtime:{}},frappe:{provide(){},
  ...
  prompt(fields,fn){submit=fn},ui:{Dialog:function(config){this.config=config;this.show=()=>{};this.hide=()=>{};dialogs.push(this)}}}};
...
(async()=>{
 ...
 x.submit({covered_hours:1,reference:'ref'});await Promise.resolve();
 ...
 x.setDirty(true);d.primary_action();assert.equal(x.calls.length,2);x.setDirty(false);d.primary_action();await Promise.resolve();
