...
## Modalidad de efectivo confirmada

El usuario respondió «Efectivo» a la pregunta sobre la compensación de la muestra. Esta decisión está resuelta y no debe preguntarse de nuevo. Se registró una nota Info no publicada en cada uno de los tres borradores mediante Comment.insert nativo, identificando la instrucción del usuario como fuente. Los borradores ya tenían planned_settlement=Cash y permanecieron intactos. Ensayo con rollback, repetición y lectura posterior pasaron; no cambiaron contadores financieros, marcaciones, reglas, elecciones ni licencias.

La nota confirma la modalidad solicitada; no representa una firma adicional del empleado ni una OverTime Settlement Election aprobada. El controlador nativo de elección exige primero un origen aprobado y una política vigente, incluso para guardar una elección Draft. No se eludió ese orden ni se aprobaron horas o reglas implícitamente. Cuando se satisfagan esas dependencias, usar la nota como referencia de la decisión ya recibida para registrar Cash por el servicio nativo. Permanecen pendientes la evidencia semanal y el tratamiento aplicable del feriado coincidente con descanso, incluida la cobertura salarial de su base.

## Liquidación nativa del sábado y elección formal

Se corrigió una dependencia excesiva: la evidencia semanal incompleta impedía liquidar incluso ventanas exclusivamente clasificadas como feriado o descanso, aunque el motor conserva esas categorías separadas de las bandas ordinarias. `weekly_bands_ready` permite solamente esos segmentos independientes con cantidades positivas, finitas y consistentes, y cero horas ordinarias clasificadas o pendientes. Las ventanas ordinarias, mixtas o desconocidas siguen exigiendo evidencia semanal completa. No modifica `weekly_evidence_complete`, no inventa sincronización y no omite política, elección, cobertura de base ni la frescura de la evidencia. Se usa tanto en la vista de conciliación como en la validación del flujo de autorización.

Pasaron 24 pruebas de semana real, siete de combinación de días y 27 de calendario. También pasaron las regresiones nativas `dev_retroactive_draft.py` y `dev_checkin_overtime.py`, ambas con rollback, incluyendo aprobación, integración con Salary Slip, rechazo de evidencia obsoleta, reintentos y reversión. El ensayo específico de la muestra pasó con rollback y repetición antes de aplicarse.

`scripts/settle_documentary_cash_sample.py` exige DEV, plan privado con hash, scheduler desactivado, dos declaraciones existentes e idénticas y referencias privadas a la confirmación Cash. Crea una política acotada al 14–16 de agosto con las reglas confirmadas, descanso en efectivo habilitado y coincidencia feriado/descanso en `Require review`. Renueva las revisiones manuales mediante token nativo y aprueba las dos jornadas. Inserta y aprueba las elecciones mediante el servicio de elección; crea el salario adicional únicamente para el sábado listo. La repetición no agrega documentos ni dinero.

Resultado guardado: sábado aprobado y liquidación Created con un Additional Salary enviado; domingo aprobado con elección Cash, pero sin importe ni salario adicional. La semana continúa incompleta en ambos. Viernes sigue Draft/Waiting por sincronización, aunque ya tiene política aplicable. No se generó Salary Slip para la muestra, ni licencia, ni ponches. Scheduler sigue desactivado. Created no es evidencia de transferencia bancaria.

La consulta de solo lectura a General Quincenal en Producción confirmó `base / 2` como salario quincenal y un componente extraordinario separado calculado con la tarifa por hora aumentada en 100%. Esto describe la configuración, no prueba una liquidación histórica ni permite afirmar por sí solo qué base de feriado fue cubierta. El usuario indicó que desconoce esa cobertura y pidió determinarla; no registrar su respuesta como cero ni volver a pedirle que adivine.

Referencia revisada: art.164 regula la opción de efectivo con recargo de 100% durante descanso semanal; art.165 trata la coincidencia de feriado y descanso; art.205 establece el recargo del feriado trabajado. La política de prueba no presenta la suma de dos recargos de 100% como conclusión legal automática. [Gaceta Oficial, arts.164–165](https://bibliotecadelcongreso.gob.do/gaceta/Gaceta_oficial_1992.pdf#page=864), [art.205](https://bibliotecadelcongreso.gob.do/gaceta/Gaceta_oficial_1992.pdf#page=871).

Rollback: identificar las fuentes mediante el recibo privado; antes de una nómina vinculada, cancelar los ajustes mediante su ciclo nativo para revertir sus salarios adicionales y liberar elecciones. No cancelar directamente el Additional Salary. Conservar historiales y crear una enmienda si hay nuevas reglas o evidencia. La política queda fijada en la fuente aprobada: un cambio posterior de regla no sustituye silenciosamente esa versión; el domingo pendiente requerirá el flujo de revisión/enmienda correspondiente. El backup de los dos archivos de ejecución está en `logs/obispo-sample-20260912/before-weekly-gate-fix.tar`. Revertir Git no revierte datos. No se ejecutó ningún cambio en Producción.

La consulta adicional de recibos del empleado seleccionado encontró una nómina enviada de la primera quincena con un Additional Salary de horas extras sin origen enlazado ni detalle de fechas. La segunda quincena permanece Draft. Importes, identificadores y respaldo están en el informe privado. Antes de trasladar la muestra a Producción, identificar qué jornadas cubre ese adicional: el respaldo no permite descartar solapamiento con el PDF. Un documento enviado no acredita un pago bancario. Esta observación no altera la prueba guardada en DEV ni autoriza nuevos importes en Producción.
